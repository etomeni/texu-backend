...
export const signupController = async (req, res, next) => {
    ...
    try {
        ...
        const userDetails = {
            userID: general.sanitizedString(await uuidv4()),
            name: req.body.name,
...
export const loginController = async (req, res, next) => {
    ...
    try {
        const uzer_ = await crud.getLimitedFirestoreDocumentData("users", 1, { property: "email", condition: "==", value: email });
        ...
        // const user = uzer_[0];
        // the code below is same as the one untop just that it removes `lastVisible` from the object
        const { lastVisible, ...user } = uzer_[0];
